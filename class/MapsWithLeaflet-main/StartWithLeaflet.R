...
leaflet() %>%                                 # create a map widget by calling the leaflet library
  addProviderTiles("OpenTopoMap") %>%         # add Esri World Physical map tiles explicitly
...
leaflet() %>%
  ...
  addProviderTiles("Thunderforest.SpinalMap",       # add custom Esri World Physical map tiles
                   options = providerTileOptions(opacity=0.4)) %>%     # make the Esri tile transparent
...
"Answer 2: The top right corner with the mapviewer is either collapsed or not: F = false and T = true "
...
leaflet() %>% 
  setView(151.2339084, -33.85089, zoom = 13) %>% # checking I am in the right area
  addTiles() %>%
  addProviderTiles("OpenStreetMap.Mapnik", group = "Hi") %>%
...
places <- read_sheet("https://docs.google.com/spreadsheets/d/1PlxsPElZML8LZKyXbqdAYeQCDIvDps2McZx1cTVWSzI/edit#gid=124710918",
                     ...
                     range = "DAMSummer2026")  # select the correct worksheet name
